...
class DonetController extends Controller
{
    public function index()
    {
        ...
        // $donasi = Donasi::get();
        $donasi = Donasi::orderBy('id', 'DESC')->get()->first();
        return view('landingpage.status', compact('donasi'));
    }

    public function indexform()
    {
        $ar_metode_pembayaran = Metode_pembayaran::all();
        return view('landingpage.donasi', compact('ar_metode_pembayaran'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('donet.index')
    ...
    public function show($id)
    {
        // $ar_jml = [10000, 25000, 50000, 100000, 1000000];
        $row = Donasi::find($id);
        return view('landingpage.detail', compact('row'));
    }
    ...
    public function bukti($id)
    {
        ...
        $donatur = Donatur::all();
        $ar_metode_pembayaran = Metode_pembayaran::all();
        return view('landingpage.status', compact('ar_metode_pembayaran', 'ar_jml'));
    }


    public function donetPDF($id)
    {
        $row = Donasi::find($id);
        $pdf = PDF::loadView('landingpage.donetPDF', ['row' => $row]);
